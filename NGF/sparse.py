...
class SparseTensor(object):
    ...
    def __init__(self,
        ...
        for i, ind in enumerate(nonsparse_indices):
            assert len(ind) == len(
                nonsparse_values
            ), 'nonsparse_indices (size{0} @index {1}) should be of same length as nonsparse_values (size {2})'.format(
        ...
        if len(self.nonsparse_values):
            self.true_shape = tuple(
                [max(inds) + 1 for inds in nonsparse_indices])
        else:
        ...
        self.lookup = np.searchsorted(
            nonsparse_indices[self.main_axis],
    ...
    def max_shape(self, max_shape):
        for true_s, max_s, in zip(self.true_shape, max_shape):
            assert (max_s is None) or (
                max_s >= true_s
            ), 'Cannot set max_shape {} smaller than true shape {}'.format(
    ...
    def shape(self):
        return tuple([
            true_s if max_s == None else max_s
    ...
    def __getitem__(self, keys):
        ...
        if isinstance(keys, (tuple, list, np.ndarray)):
            if len(keys) == 0:
                raise IndexError(
                    'Cannot index `SparseTensor` with empty slice (`[]`)')
            else:
                assert isinstance(
                    keys[0], int
                ), 'Indexing is only allowed along the main axis ({})'.format(
                    self.main_axis)
        elif isinstance(keys, int):
            pass
        else:
            raise IndexError(
                'Only int, list, np.ndarray or slice (`:`) allowed for indexing `SparseTensor`'
            )
        ...
        return self.__class__(
            dtype=self.dtype,
            nonsparse_indices=indices,
            nonsparse_values=values,
            main_axis=main_axis,
    ...
    def from_array(cls,
        ...
        return cls(
            dtype=arr.dtype,
            nonsparse_indices=nonsparse_indices,
            nonsparse_values=nonsparse_values,
            main_axis=0,
            max_shape=max_shape,
    ...
    def as_array(self, shape=None):
        ...
        shape = [
            true_s if s == None else s for s, true_s in zip(shape, self.shape)
        ]
        # Check if obtained shape matches with self.true_shape
        assert np.all([
            s >= true_s for s, true_s in zip(shape, self.true_shape)
        ]), 'shape ({}) should be at least {}'.format(shape, self.true_shape)
    ...
    def from_config(cls, config):
        ...
        '''
        return cls(
            nonsparse_indices=config['nonsparse_indices'],
            nonsparse_values=config['nonsparse_values'],
            default_value=config['default_value'],
            dtype=config['dtype'],
            main_axis=config['main_axis'],
...
class GraphTensor(TensorList):
    ...
    def __init__(self,
        ...
        max_shapes = mol_dims_to_shapes(max_atoms, max_degree,
        ...
        edges = SparseTensor.from_array(
            edges, max_shape=max_shapes[2], default_value=-1)
...
def unit_tests_sparse_tensor(seed=None):
    ...
    assert np.all(sparse[len(sparse) - 1].as_array(singleton_shape) == sparse[
        -1].as_array(singleton_shape))
    ...
    print(
        'Testing: Various indexing testcases that should return same array as sparse'
    )
    assert np.all(
        sparse.as_array(full_shape) == sparse[:].as_array(full_shape))
    assert np.all(
        sparse.as_array(full_shape) == sparse[0:len(sparse) + 10].as_array(
            full_shape))

    print(
        'Testing: Equality functions return `True` for all entries when comparing sparse with sparse'
    )
    assert np.all(sparse == sparse.as_array(full_shape))
    # assert np.all(sparse.as_array(full_shape) == sparse)

    print(
        'Testing: Equality functions return `True` for all entries when comparing sparse with original array'
    )
    assert np.all(arr == sparse.as_array(full_shape))
    # assert np.all(sparse.as_array(full_shape) == arr)

    print(
        'Testing: Equality functions should return same boolean array as numpy'
    )
    assert np.all((arr[0] == 0) == (sparse[0] == 0))
    assert np.all((arr[0] == arr[3]) == (sparse[0] == sparse[3]))

    print(
        'Testing: Inequality functions return `False` for all entries when comparing sparse with sparse'
    )
    assert not np.all(sparse != sparse.as_array(full_shape))
    # assert not np.all(sparse.as_array(full_shape) != sparse)

    print(
        'Testing: Inequality functions return `False` for all entries when comparing sparse with original array'
    )
    assert not np.all(arr != sparse.as_array(full_shape))
    assert not np.all(sparse.as_array(full_shape) != arr)

    print(
        'Testing: Ineuality functions should return same boolean array as numpy'
    )
    assert np.all((arr[0] != 0) == (sparse[0] != 0))
    assert np.all((arr[0] != arr[3]) == (sparse[0] != sparse[3]))
...
def unit_tests_graph_tensor(seed=None):
    ...
    for i, n_atoms in enumerate(
            np.random.randint(1, max_atoms, size=num_molecules)):
        atoms[i, 0:n_atoms, :] = np.random.randint(
            3, size=(n_atoms, num_atom_features))

        # Generator neighbours/bonds for each atom
        for a, degree in enumerate(
                np.random.randint(max_degree, size=n_atoms)):
            bonds[i, a, 0:degree, :] = np.random.randint(
                3, size=(degree, num_bond_features))
            edges[i, a, 0:degree] = np.random.randint(max_degree, size=degree)

    mols = GraphTensor(
        [atoms, bonds, edges], sparse_max_atoms=True, sparse_max_degree=True)
...
def unit_test_epoch_iterator(seed=None):
    ...
    assert np.all(iter_epochs == true_epoch1) or np.all(
        iter_epochs == true_epoch2)
